...
这里 PC1 R1 R3 PC2 都由助教提供，你的树莓派在 R2 的位置。初始情况下，R1 和 R3 先不启动 RIP 协议处理程序，这些机器的路由表如下：
...
此时，接上你的树莓派，按照图示配置后，从 PC1 到 PC2 是不通的。接着，在 R1 和 R3 上都开启 RIP 协议处理程序，它们分别会在自己的 RIP 包中宣告 `192.168.1.0/24` 和 `192.168.5.0/24` 的路由。一段时间后路由表应该变成这样：
...
这个时候，PC1 和 PC2 应当是互通的，R1 和 R3 也应当是互通的，我们会进行 ping 和 iperf3 的测试；也在整个过程中在 R1 和 R3 上观察你的路由器发出的 RIP 包是否实现了要求；还会检查树莓派上的运行状态，确保你是采用 HAL 进行转发，而不是用 Linux 自带的功能。
