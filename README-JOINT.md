...
4. 双链接双工：在 PC2 运行 `iperf -s`，在 PC3 运行 `iperf -s`，同时在 PC1 运行 `iperf -c 192.168.6.3 -d -t 10` 和在 PC4 运行 `iperf -c 192.168.7.3 -d -t 10`，默认参数运行，带宽之和不低于 150 Mbps（20%），不低于 300Mbps （25%），不低于 350Mbps（30%）
